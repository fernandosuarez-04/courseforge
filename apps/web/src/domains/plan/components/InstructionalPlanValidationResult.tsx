    ...
    return (
        <div className="space-y-2">
            <div className="flex justify-between text-xs font-semibold uppercase tracking-wide text-gray-400">
                <span>{label}</span>
                <span className={value >= 90 ? "text-[#00D4B3]" : "text-white"}>{value}%</span>
            </div>
...
export function InstructionalPlanValidationResult({ validation }: { validation: any }) {
    ...
    return (
        <div className="space-y-4 animate-in fade-in slide-in-from-top-4 duration-700">
            ...
            {/* Main Decision Card */}
            <div className="rounded-xl border border-gray-800 bg-[#0A0E12] overflow-hidden shadow-2xl shadow-black/50">
                
                {/* Header Section */}
                <div className="p-6 border-b border-gray-800 flex flex-col md:flex-row justify-between items-start gap-6 relative overflow-hidden">
                    {/* Background Glow */}
                    ...
                    <div className="text-right hidden md:block shrink-0 z-10 pl-6 border-l border-gray-800/50">
                ...
                {/* Metrics Grid */}
                <div className="p-8 bg-[#0F1419]/30">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 divide-y md:divide-y-0 md:divide-x divide-gray-800 border-t border-gray-800">
                    {/* Fortalezas */}
                    <div className="p-6 space-y-4 bg-[#00D4B3]/[0.02]">
                        ...
                        <ul className="space-y-3">
                            ...
                            )) : <span className="text-gray-600 text-sm">No se detectaron fortalezas específicas.</span>}
                        </ul>
                    </div>
                    
                    {/* Recomendaciones */}
                    <div className="p-6 space-y-4 bg-orange-500/[0.02]">
                        <div className="flex items-center gap-2 text-orange-400 font-semibold text-sm uppercase tracking-wide mb-2">
                            <TrendingUp size={16} />
                             Recomendaciones
                        </div>
                        <ul className="space-y-3">
                            ...
                            )) : <span className="text-gray-600 text-sm">No hay recomendaciones críticas.</span>}
                        </ul>
                    </div>
                </div>
            </div>
